...
| ----------------------- | ----------------- | ------------------------------------------------------------------------------------------------ |
| `UserRole`            | —                | Union `"admin" \| "artists" \| "news"` — miroir du type ENUM PostgreSQL `user_role`          |
| `UserItem`            | —                | Ligne utilisateur complète retournée par l'API — id, email, display_name, role, created_at, password_changed_at |
| `AdminUser`           | `Omit<UserItem>` | Utilisateur connecté sans `created_at` ni `password_changed_at` — retourné par `GET /admin/auth/me` |
...
| `content`            | `string \| null` | Corps de la news en HTML — absent des réponses de liste        |
...
| Type           | Dérivé de        | Description                                                                                                        |
| -------------- | ---------------- | ------------------------------------------------------------------------------------------------------------------ |
| `ArtistItem`  | —               | Artiste complet retourné par l'API — tous les champs y compris bio, genre, origin                                |
...
| Type       | Description                                                                                                                      |
| ---------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `NavItem` | Élément de navigation ou de filtre — `label`, `labelBtn`, `path`, `active`, `value`, `role`, `desc`, `onClick` tous optionnels |

Utilisé par `ui.ts` pour définir les items de navigation (`navVisitorItems`, `navAdminItem`…) et les filtres (`filterArtistsItems`, `filterNewsItems`, `filterUsersItems`).
...
| Type                     | Description                                                                                               |
| ------------------------ | --------------------------------------------------------------------------------------------------------- |
| `ApiMessageResponse`   | Réponse générique `{ message?: string }` — utilisée pour les endpoints qui ne retournent qu'un message |
| `CreateApiResponse<T>` | Réponse de création ou modification — `{ message: string } & T` — le type `T` est l'entité retournée  |
