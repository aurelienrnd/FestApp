...
export async function login(req: Request, res: Response) {
  ...
  try {
    ...
    //NOTE modiffier Date.now() + "REFRESH_TOKEN_EXPIRES_IN in dateTime Type
    await query(
    ...
    const repSessions = await query(
      `SELECT * FROM sessions WHERE $1 = user_id ORDER BY created_at DESC LIMIT 1`,
      [user.id],
    );
